...
const scholarName = 'Dr. RuiDong Qi (祁瑞东)';

export default function HomeHero() {
  const academicLinks = getPiAcademicLinks();

  return (
    <section className="home-hero-tech relative mb-10 overflow-hidden rounded-xl border-2 border-border/80 bg-card p-6 md:p-10 luxury-card tech-frame-ambient page-section-reveal">
      <div className="relative z-10 flex flex-col lg:flex-row items-center gap-8 lg:gap-12">
        <div className="flex shrink-0 flex-col items-center">
          <div className="relative">
            ...
            <Image
              ...
              alt={`Portrait of ${scholarName}`}
              width={200}
              height={300}
              className="mt-4 rounded-none object-cover h-64 w-44 border-2 border-border lg:h-80 lg:w-56"
              priority
            />
          </div>
        </div>
        ...
        <div className="flex-1 text-center lg:text-left max-w-3xl">
          ...
          <h1 className="text-3xl md:text-4xl lg:text-5xl font-bold leading-tight text-gradient-luxury mb-4">
            {scholarName}
          </h1>
          <p className="text-lg md:text-xl text-foreground/90 font-medium leading-snug mb-3">
            {homeHero.tagline}
          </p>
          ...
          <p className="text-xs text-muted-foreground flex flex-wrap gap-x-3 gap-y-1 justify-center lg:justify-start">
            {academicLinks.map((link, i) => (
              <span key={link.name}>
                {i > 0 && <span className="text-border mr-3 hidden sm:inline" aria-hidden>·</span>}
                <a
                  href={link.url}
                  target="_blank"
            ...
            <span className="w-full sm:w-auto sm:ml-1">
